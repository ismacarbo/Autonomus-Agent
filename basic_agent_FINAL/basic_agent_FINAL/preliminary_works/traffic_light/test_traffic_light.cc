    ...
    while (server_run == 1) {
        ...
        if (server_receive_from_client(&server_run, &message_id, &scenario_msg.data_struct) == 0) {
            ...
            if (in->TrfLightCurrState == 1 &&
                ...
                time_to_green = 0;
                time_to_red = in->TrfLightFirstTimeToChange;
                phase = 1;
            }
            ...
            if (phase == 0 && in->TrfLightCurrState == 2 &&
                ...
                time_to_green = in->TrfLightSecondTimeToChange;
                time_to_red = in->TrfLightThirdTimeToChange;
                phase = 2;
            }
            ...
            if (phase == 0 && in->TrfLightCurrState == 3 &&
                ...
                time_to_green = in->TrfLightFirstTimeToChange;
                time_to_red = in->TrfLightSecondTimeToChange;
                phase = 3;
            }
            ...
            tmin = time_to_green;
            tmax = time_to_red;
            pass_primitive(vel, acc, sf, vmin, vmax, time_to_green, time_to_red,
            ...
            // This approach is not robust enough
            //double req_acc = a_opt(DT,vel,acc,bests,bestv,0,bestT);
            ...
            logger.log_var("logout", "phase", in->TrfLightCurrState);
            logger.log_var("logout", "trt1", in->TrfLightFirstTimeToChange);
            logger.log_var("logout", "trt2", in->TrfLightSecondTimeToChange);
            logger.log_var("logout", "trt3", in->TrfLightThirdTimeToChange);
