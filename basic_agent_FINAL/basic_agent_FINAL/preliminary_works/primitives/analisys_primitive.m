...
%% TEST 3 Pass Primitive and Stop Primitive
% Time Plot
figure(3);
...
for ind = 1:10:length(myd.time)
    ...
    if myd.type(ind) < 0
        if myd.type(ind) == -2
            plot(times_primitive_vect,vel_primitive_vect,'-r','LineWidth',1,'HandleVisibility','off');
        else
            plot(times_primitive_vect,vel_primitive_vect,'-r','LineWidth',0.5,'HandleVisibility','off');
        end
    else
        if myd.type(ind) == 3
            plot(times_primitive_vect,vel_primitive_vect,'-g','LineWidth',2,'HandleVisibility','off');
        else
...
for ind = 1:10:length(myd.time)
    ...
    if  myd.type(ind) < 0
        if myd.type(ind) == -2
            plot(pos_primitive_vect+myd.dist(ind),vel_primitive_vect,'-r','LineWidth',1,'HandleVisibility','off');
        else
            plot(pos_primitive_vect+myd.dist(ind),vel_primitive_vect,'-r','LineWidth',0.5,'HandleVisibility','off');
        end
    else
        if myd.type(ind) == 3
            plot(pos_primitive_vect+myd.dist(ind),vel_primitive_vect,'-g','LineWidth',2,'HandleVisibility','off');
        else
