...
const uint16_t DEFAULT_VELOCITY_KP_Q8 = 90U;
const uint16_t DEFAULT_VELOCITY_KI_Q8 = 128U;
const uint16_t DEFAULT_VELOCITY_FF_Q8 = 160U;
const uint16_t DEFAULT_ENCODER_TICKS_PER_REV = 2400U;
// Effective drive radius remains provisional until the sprocket is measured.
const uint32_t DEFAULT_WHEEL_RADIUS_UM = 32000UL;
...
void update_velocity_control() {
  ...
  const int32_t delta_l = ticks_l - last_velocity_ticks_l;
  const int32_t delta_r = ticks_r - last_velocity_ticks_r;
  last_velocity_ticks_l = ticks_l;
  last_velocity_ticks_r = ticks_r;
  ...
  measured_velocity_l_mm_s = delta_l * distance_per_tick_mm * inv_dt;
  measured_velocity_r_mm_s = delta_r * distance_per_tick_mm * inv_dt;
...
void update_motors() {
  ...
  if (allow_motion) update_velocity_control();
